		...
		for (Entry<InetAddress, Player> e : players.entrySet()) {
			if (playersAlive.containsKey(e.getKey()) || players.size() == 1) {
				...
				if (!b.isDead()) {
					...
					if (xPos < 0.5 + b.getRadius() || xPos > midX * 2 - (0.5 + b.getRadius())
							|| yPos < 0.5 + b.getRadius() || yPos > midY * 2 - (0.5 + b.getRadius())) {
					...
					// Calculate and apply force to body depending on mouse
					// position
